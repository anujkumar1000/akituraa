...
export function CartDrawer() {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "absolute inset-0 bg-lav-900/40 transition-opacity duration-300",
      ...
      >
        <header className="flex items-center justify-between border-b border-lav-200 px-5 py-4">
          ...
          <button
            ...
            className="grid h-9 w-9 place-items-center rounded-full hover:bg-lav-100"
          >
            <X className="h-5 w-5" />
          </button>
        </header>
        ...
        {lines.length > 0 && (
          <div className="border-b border-lav-100 px-5 py-3">
            <p className="text-xs text-lav-700">
              {remaining > 0 ? (
                <>
                  You&apos;re <strong>{formatPrice(remaining)}</strong> away
                  from free shipping! 🚚
                </>
              ) : (
                <>
            ...
            <div className="mt-2 h-2 overflow-hidden rounded-full bg-lav-150">
              <div
                className="h-full rounded-full bg-gradient-to-r from-lav-500 to-blush-deep transition-all duration-500"
          ...
          {lines.length === 0 ? (
            <div className="grid h-full place-items-center text-center">
              <div>
                ...
                <p className="mt-3 font-display font-semibold text-lav-900">
                  Your bag is empty
                </p>
                ...
                <li key={l.id} className="flex gap-3">
                  ...
                  <div className="flex flex-1 flex-col">
                    <Link
                      href={`/products/${l.slug}`}
                      onClick={close}
                      className="line-clamp-1 font-display text-sm font-semibold text-lav-900"
                    >
                      {l.name}
                    </Link>
                    {l.variant && (
                      <span className="text-xs text-muted">{l.variant}</span>
                    )}
                    <span className="text-sm font-bold text-lav-700">
                    ...
                    <div className="mt-auto flex items-center justify-between">
                      <div className="flex items-center rounded-full border border-lav-200">
                        <button
                          ...
                          className="grid h-8 w-8 place-items-center rounded-full hover:bg-lav-100"
                          aria-label="Decrease"
                        >
                        ...
                        <button
                          ...
                          className="grid h-8 w-8 place-items-center rounded-full hover:bg-lav-100 disabled:opacity-40"
                          aria-label="Increase"
                        >
                      ...
                      <button
                        onClick={() => remove(l.id)}
                        aria-label="Remove"
                        className="text-muted hover:text-blush-deep"
                      >
        ...
        {lines.length > 0 && (
          <footer className="border-t border-lav-200 px-5 py-4">
            ...
            <Link
              href="/checkout"
              onClick={close}
              className="mt-3 flex w-full items-center justify-center rounded-full bg-lav-600 px-6 py-3 font-semibold text-white transition hover:bg-lav-700"
            >
              Checkout
            </Link>
            <button
              onClick={close}
              className="mt-2 w-full text-center text-sm font-semibold text-lav-600 hover:text-lav-800"
            >
              Continue shopping
            </button>
          </footer>
