...
function tabLabel(pathname: string) {
  ...
  return TAB_LABELS.find(([p]) => pathname.startsWith(p))?.[1] ?? "AKITAURAA.EXE";
}
...
function Clock() {
  ...
  return <span className="font-nav text-[11px] uppercase tracking-wide text-xp-ink">{time}</span>;
}
    ...
    <div className="fixed inset-x-0 bottom-0 z-[60] border-t border-xp-ink-2 bg-xp-titlebar">
      <div className="mx-auto flex h-10 max-w-[1440px] items-center gap-2 px-2 sm:px-3">
        ...
        <span className="hidden h-7 items-center gap-1.5 rounded-[2px] border border-xp-ink-2 bg-xp-inset px-3 font-nav text-[10px] uppercase tracking-wide text-xp-ink sm:flex">
          {tabLabel(pathname)} <Heart className="h-3 w-3 fill-xp-pink text-xp-pink" />
        </span>
...
export function Navbar({ categories }: { categories: Category[] }) {
  ...
  const nav = [
    { label: "HOME", href: "/" },
    { label: "SHOP", href: "/shop", dropdown: categories.map((c) => ({ label: c.name.toUpperCase(), href: `/category/${c.slug}` })) },
    {
      label: "COLLECTIONS", href: "/shop?featured=1",
      dropdown: [
      ...
      <div className="mx-auto flex h-[72px] max-w-[1440px] items-center gap-3 px-3 sm:px-5">
        ...
        <Link href="/" className="relative flex flex-col leading-none">
          <span className="relative font-logo text-3xl font-bold  tracking-tight text-xp-ink sm:text-4xl">
            Akitauraa
            <svg aria-hidden viewBox="0 0 40 40" className="absolute -right-7 -top-2 h-7 w-7 text-xp-accent">
              <path d="M20 4 l2.5 6.5 L29 13 l-6.5 2.5 L20 22 l-2.5-6.5 L11 13 l6.5-2.5 Z" fill="currentColor" />
              <path d="M32 24 l1.4 3.6 3.6 1.4 -3.6 1.4 -1.4 3.6 -1.4-3.6 -3.6-1.4 3.6-1.4 Z" fill="currentColor" opacity="0.7" />
            </svg>
            <svg aria-hidden viewBox="0 0 120 44" className="pointer-events-none absolute -left-3 -top-2 h-[44px] w-[120%] text-xp-ink-2 opacity-40">
              <ellipse cx="60" cy="22" rx="58" ry="16" fill="none" stroke="currentColor" strokeWidth="1" strokeDasharray="3 4" />
            </svg>
          </span>
        ...
        <div className="ml-auto flex items-center gap-2 lg:ml-0">
          <form onSubmit={submitSearch} className="relative hidden md:block">
            ...
            />
            <button type="submit" aria-label="Search" className="absolute right-1.5 top-1/2 -translate-y-1/2 text-xp-ink-2 hover:text-xp-ink">
              <Search className="h-4 w-4" />
            </button>
          </form>

          <Link href="/account" aria-label="Account" className="grid h-9 w-9 place-items-center rounded-[2px] text-xp-ink-2 hover:bg-xp-panel hover:text-xp-ink">
            <User className="h-4.5 w-4.5" />
          </Link>
          <Link href="/wishlist" aria-label="Wishlist" className="flex items-center gap-1 rounded-[2px] px-1.5 py-1 text-xp-ink-2 hover:bg-xp-panel hover:text-xp-ink">
            <Heart className="h-4.5 w-4.5" />
            <span className="font-nav text-[11px]">({mounted ? wishCount : 0})</span>
          </Link>
          <button onClick={openCart} aria-label="Open cart" className="flex items-center gap-1 rounded-[2px] px-1.5 py-1 text-xp-ink-2 hover:bg-xp-panel hover:text-xp-ink cursor-pointer">
            <ShoppingBag className="h-4.5 w-4.5" />
            <span className="font-nav text-[11px]">({mounted ? cartCount : 0})</span>
          </button>
        </div>
      </div>

      {/* mobile drawer */}
      <div className={cn("fixed inset-0 z-[70] lg:hidden", menuOpen ? "pointer-events-auto" : "pointer-events-none")}>
        <div className={cn("absolute inset-0 bg-xp-ink/40 transition-opacity", menuOpen ? "opacity-100" : "opacity-0")} onClick={() => setMenuOpen(false)} />
        <aside className={cn(
          ...
          <div className="xp-titlebar -m-3 mb-3 flex h-9 items-center gap-2 border-b border-xp-ink-2 px-3">
            <span className="font-nav text-[11px] font-semibold uppercase tracking-widest text-xp-ink">-- MENU.EXE</span>
            <button onClick={() => setMenuOpen(false)} aria-label="Close menu" className="ml-auto grid h-5 w-5 place-items-center rounded-[2px] border border-xp-ink-2 bg-xp-panel text-[10px]">
              <X className="h-3 w-3" />
            </button>
          </div>
          <nav onClick={() => setMenuOpen(false)} className="flex flex-col gap-0.5">
            {nav.map((n) => (
              <Link key={n.label} href={n.href} className="rounded-[2px] px-3 py-2.5 font-nav text-[12px] uppercase tracking-[0.12em] text-xp-ink hover:bg-xp-highlight">
                {n.label}
              </Link>
            ))}
            <p className="mt-3 px-3 font-nav text-[9px] uppercase tracking-widest text-xp-muted">categories</p>
            {categories.map((c) => (
              <Link key={c.id} href={`/category/${c.slug}`} className="rounded-[2px] px-3 py-2 font-nav text-[11px] uppercase tracking-[0.12em] text-xp-ink-2 hover:bg-xp-highlight hover:text-xp-ink">
                {c.name}
              </Link>
            ))}
          </nav>
