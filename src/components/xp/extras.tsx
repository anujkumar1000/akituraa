    ...
    <div className="xp-window overflow-hidden rounded-[3px] bg-xp-panel py-2.5">
      <div className="flex w-max animate-marquee items-center gap-6 whitespace-nowrap">
        {track.map((t, i) => (
          <span
            key={i}
            className="flex items-center gap-6 font-nav text-[10px] font-medium uppercase tracking-[0.16em] text-xp-ink-2"
          >
            {t}
            <span aria-hidden className="text-xp-accent">
              ✦·˙
            </span>
          </span>
        ))}
      ...
      {CARDS.map((c, i) => {
        ...
        return (
          <Link
            key={c.label}
            href={c.href}
            className="group xp-window overflow-hidden rounded-[3px] transition-transform duration-200 hover:-translate-y-0.5"
          >
            <div className="flex items-center justify-between px-3 py-2.5">
              <span className="font-nav text-[12px] font-semibold uppercase tracking-[0.14em] text-xp-ink">
                {c.label}
              </span>
